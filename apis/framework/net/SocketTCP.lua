...
local SOCKET_TICK_TIME = 0.2 -- check socket data interval
...
function SocketTCP:ctor(__host, __port, __retryConnectWhenFailure)
    --require("framework.components.behavior.EventProtocol").extend(self)
...
function SocketTCP:send(__data)
    ...
    --[[if not self.isConnected then
        app.msgBox.showMsgBox("连接已断开,请重新登录", function()
            cc.dataMgr.isChangeAccLogin = false
            app.sceneSwitcher:enterScene("LoginScene")
        end)
    else]]
         local r = self.tcp:send(__data)
    --end
...
function SocketTCP:_disconnect()
    ...
    self.isConnected = false
    self.tcp:shutdown()
    self.eventProtocol:dispatchEvent({ name = SocketTCP.EVENT_CLOSED, socket = self })
end
...
function SocketTCP:_onConnected()
    ...
    local __tick = function()
        while true do
            ---[[
            local _recvt, _sendt, _status = socket.select({ self.tcp }, nil, 0)
            --print("_recvt:", #_recvt, "_sendt:", _sendt, "_status:", _status)

            if #_recvt ~= 0 and _status ~= "timeout" then
                
                _closeTrick = _closeTrick - 1
            else
                _closeTrick = 5
            end
            if _closeTrick == 0 then
                --[[ local function clickOK()
                    cc.dataMgr.isChangeAccLogin = true
                    app.sceneSwitcher:enterScene("LoginScene")
                end
                app.msgBox.showMsgBoxEx({strMsg = "服务器无回应断开连接---测试", funcOk = clickOK, isHideX = true})]]
                --self:close()
            ...
            --if #_recvt > 0 and _status ~= nil then
            ...
            if __status == STATUS_CLOSED or __status == STATUS_NOT_CONNECTED  or __status == "reset" then
                --if __body and __partial then __body = __body .. __partial end
                --self.eventProtocol:dispatchEvent({ name = SocketTCP.EVENT_DATA, data = (__partial or __body), partial = __partial, body = __body })
          --   app.msgBox.showMsgBoxEx({strMsg = __status .."123123"})
                self:close()
                if self.isConnected then
                    self:_onDisconnect()
                else
...
function SocketTCP:_reconnect(__immediately)
    if not self.isRetryConnect then return
    end
    print(self.name .. "_reconnect")
    if __immediately then self:connect() return
    end
    if self.reconnectScheduler then scheduler.unscheduleGlobal(self.reconnectScheduler)
    end
